...
const CalendarWorkspace: React.FC<CalendarWorkspaceProps> = ({ tasks, onComplete }) => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 min-h-0 bg-[#cccccc] p-[10px]">
        <div className="h-full grid grid-cols-7 grid-rows-6 gap-[6px]">
          {Array.from({ length: totalSlots }).map((_, slot) => {
            ...
            if (d < 1 || d > daysInMonth) {
              return <div key={`e-${slot}`} className="bg-[#cccccc]" />;
            }
          ...
          let cls = 'flex flex-col items-center justify-center gap-[2px] text-[14px] cursor-pointer transition-colors';
          if (isSelected) cls += ' bg-[#f8f8f8] text-foreground';
          else if (isToday) cls += ' bg-[#eeeeee] text-foreground';
          else if (allDone) cls += ' bg-[#c0cfbf] text-foreground/80';
          else if (hasBreak) cls += ' bg-[#cec8b8] text-foreground/75';
          else if (hasTasks && !isFuture) cls += ' bg-[#c4c4c4] text-foreground/75 hover:bg-[#c9c9c9]';
          else cls += ' bg-[#cccccc] text-foreground/65 hover:bg-[#d0d0d0]';
